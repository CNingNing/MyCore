...
namespace Winner.Message
{
    ...
    public class DistributedMessage : IMessage
    {
        ...
        {
            var value = Handle('s', new MessageProtocolInfo { Key = info.Key, Count=info.Count,ExpireSecond=info.ExpireSecond });
            if (value == null)
                return false;
        ...
        {
            var value =Handle('a', new MessageProtocolInfo { Key = info.Key, Count = info.Count, ExpireSecond = info.ExpireSecond });
            if (value == null)
                return 0;
        ...
        {
            var value = Handle('r', new MessageProtocolInfo { Key = info.Key, Count = info.Count, ExpireSecond = info.ExpireSecond });
            if (value == null)
                return 0;
            return int.Parse(value);
        }
        ...
        public virtual int GetCount(string key)
        {
            if (string.IsNullOrWhiteSpace(key))
                return 0;
            string rev = Handle('g', new MessageProtocolInfo { Key = key});
        ...
        private const string ChannelName = "MessageService";
        ...
        /// <returns></returns>
        protected virtual string Handle(char method, MessageProtocolInfo protocol)
        {
