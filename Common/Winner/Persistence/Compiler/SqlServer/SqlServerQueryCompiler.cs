...
namespace Winner.Persistence.Compiler.SqlServer
{
    public class SqlServerQueryCompiler : QueryCompiler
    {
        ...
        /// 查询实例
        /// </summary>
        public override ISelectCompiler SelectCompiler { get; set; }=new SqlServerSelectCompiler();

        /// <summary>
        /// 查询实例
        /// </summary>
        public override IWhereCompiler WhereCompiler { get; set; } = new SqlServerWhereCompiler();
            ...
            string groupbyExp, string havingExp, string orderbyExp, QueryInfo query)
        {
            ...
            sql.Append(string.Format(") t  where Auto_RowNumberIndex between {0} and {1} order by Auto_RowNumberIndex asc", start, end));
        }
